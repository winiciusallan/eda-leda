...
class valorMaisProximoDeN {
    public static void main(String[] args) {
        ...
        System.out.println(Arrays.toString(preOrderArray(bst).toArray()));
        System.out.println(foundCloser(bst, n));
        input.close();
    }

    private static int foundCloser(BST bst, int n) {
        return foundCloser(bst.getRoot(), n, bst.getRoot().value);
    }

    private static int foundCloser(Node current, int n, int closer) {
        if (current == null)
            return closer;

        int currDiff = Math.abs(n - current.value);
        int closerDiff = Math.abs(n - closer);
        if (currDiff <= closerDiff) {
            if (current.value < n) {
                return foundCloser(current.right, n, current.value);
            }

            if (current.value > n) {
                return foundCloser(current.left, n, current.value);
            }
        } else {
            if (current.value < n) {
                return foundCloser(current.right, n, closer);
            }

            if (current.value > n) {
                return foundCloser(current.left, n, closer);
            }
        }
    ...
    private static ArrayList<Integer> preOrderArray(BST bst) {
        ArrayList<Integer> arr = new ArrayList<>();
        preOrder(bst.getRoot(), arr);
        return arr;
    }

    private static void preOrder(Node curr, ArrayList<Integer> arr) {
        if (curr != null) {
            arr.add(curr.value);
    ...
    private static void createBst(BST bst, int[] v) {
        for (int elem : v) {
            bst.add(elem);
        }
    }
...
class BST {
     ...
     * @return O nó contendo o predecessor do valor passado como parâmetro. O método
     ...
     * @return O nó contendo o sucessor do valor passado como parâmetro. O método
     *         retorna null
    ...
    /**
     * Busca o nó cujo valor é igual ao passado como parâmetro na sub-árvore cuja
     * raiz é node. Trata-se de um método auxiliar
