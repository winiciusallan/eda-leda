...
public class TestFloorBinarySearch {
    
    private FloorBinarySearchImpl implementation;
    private Integer[] arrPar;

    @Before
    public void setUp() {
        this.implementation = new FloorBinarySearchImpl();
        this.arrPar = new Integer[]{4,6,8,10};
    }

    @Test
    public void testaBuscaBinaria() {
        int pos = this.implementation.floor(arrPar, 7);

        assertEquals(6, pos);
    }

    private void genericTest(Integer[] array, Integer x, Integer expected) {
        assertEquals(expected, implementation.floor(array, x));
    }
